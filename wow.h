...
#include "StereoWaveFX.h"
#include "StereoChorusFX.h"
#include "StereoAMFX.h"
...
public:
	...
	CMiniSynthVoice* getOldestNotLowestVoice();
	CMiniSynthVoice* getOldestNotLowestVoiceWithNote(UINT uMIDINote);
	...
	// --- Mod Matrix Routing
	//void decodeModRoute();
	void enableModRoute(UINT source, UINT & newConnection, UINT & oldConnection);

	//SOURCE, UINT new connection, UINT old connection(index[x][y]),
	UINT m_uConnections[4][3];
	UINT m_uDestinations[8];
	void disableEGtoAmp(int EGindex, UINT oldConnection);

	void setModIntensityALL();
	void setModIntensity(UINT source, UINT destination, double i);
	double dummy;
	double * m_dIntensityMatrix[4][8];

	// --- FX
	CStereoDelayFX m_DelayFX;
	StereoWaveFX m_WaveFX;
	CStereoAMFX m_AMFX;
	CStereoChorusFX m_ChorusFX;

	// --- Indexing WaveForms
	UINT m_uAMMul[4];

	// --- Indexing Filter Types
	UINT m_uOscTypes[6];
	UINT m_uFilterTypes[6];
